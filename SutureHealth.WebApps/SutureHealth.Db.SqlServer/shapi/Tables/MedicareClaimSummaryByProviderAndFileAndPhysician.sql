CREATE TABLE [shapi].[MedicareClaimSummaryByProviderAndFileAndPhysician] (
    ...
    CONSTRAINT [FK_provider_medicare_claims_by_provider_and_file_and_physician_provider_provider_id] FOREIGN KEY ([ProviderId]) REFERENCES [shapi].[Provider] ([ProviderId])
);
...
CREATE NONCLUSTERED INDEX [IX_provider_medicare_claims_by_provider_and_file_and_physician_provider_organization_npi_file_physician_npi]
    ON [shapi].[MedicareClaimSummaryByProviderAndFileAndPhysician]([ProviderOrganizationNPI] ASC, [File] ASC, [PhysicianNPI] ASC);
