...
CREATE NONCLUSTERED INDEX [IX_provider_npi]
    ON [shapi].[Provider]([NPI] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_provider_suture_facility_id]
    ON [shapi].[Provider]([SutureFacilityId] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_provider_provider_primary_facility_id]
    ON [shapi].[Provider]([SuturePrimaryFacilityId] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_provider_suture_user_id]
    ON [shapi].[Provider]([SutureUserId] ASC);


GO
CREATE NONCLUSTERED INDEX [IX_network_provider_default_index]
    ON [shapi].[Provider]([SutureCreatedAt]) INCLUDE ([Latitude],[Longitude],[SutureCustomerType],[IsNetworkProvider],[SutureCloseDate]);

GO

CREATE NONCLUSTERED INDEX [IX_ProviderNetworkProviderCloseDateIndex]
    ON [shapi].[Provider]([IsNetworkProvider] ASC,[SutureCloseDate] ASC) INCLUDE([ProviderType],[AddressLine1],[AddressLine2],[City],[State],[PostalCode],[PostalCodeArea],[EmailAddress],[TelephoneNumber],[FaxNumber],[FirstName],[MiddleName],[LastName],[Suffix],[ProfessionalSuffix],[Name],[SigningName],[Latitude],[Longitude],[ServiceCount],[NPI],[IsActiveInSutureSign],[IsCollaborator],[IsNPIActive],[IsSender],[IsSigner],[CanSign],[SutureCreatedAt],[SutureUpdatedAt],[SutureCustomerType],[SutureFacilityId],[SutureFacilityTypeId],[SuturePrimaryFacilityId],[SuturePrimaryFacilityName],[SutureUserId],[SutureUserTypeId])
